...
the Katakana script, I<etc>.  (C<Script_Extensions> is an improved
version of C<Script>, which is retained for backward compatibility, and
so you should generally use C<Script_Extensions>.)  You can test whether
a character is in a particular script  with, for example C<\p{Latin}>,
C<\p{Greek}>, or C<\p{Katakana}>.  To test if it isn't in the Balinese
script, you would use C<\P{Balinese}>.  (These all use
C<Script_Extensions> under the hood, as that gives better results.)
