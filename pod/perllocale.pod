...
WARNING!  Caveats apply to using this function in a L<thread|threads>.
See L</Multi-threaded operation>.
...
Multi-threaded locale operation is supported in later Perl versions.
This started with Perl 5.28, and was extended in Perl 5.34.

The 5.28 support is limited to Windows Visual Studio 2005 and later, and
on systems that implement the POSIX 2008 thread-safe locale operations.
Many modern systems, such as many Unix variants and Darwin, have
these.

5.34 adds support for pure perl programs and those whose XS code is
well-behaved (see below), by emulating thread-safe operation on
platforms that don't actually have it.  This makes many programs
portable that between 5.28 and 5.32 were not.

The read-only boolean variable C<${^SAFE_LOCALES}> can be used to see if the particular Perl is .  The value is 1 if the
perl is not threaded, or if it is using thread-safe locale operations.
This variable is a lot less useful starting in 5.34.

Some platforms claim to support POSIX 2008, but have buggy
implementations, so that the hints files for compiling to run on them
turn off attempting to use that form of thread-safety.
C<${^SAFE_LOCALES}> will be 0 on them.

You automatically get thread-safe behavior for threaded perls, without
having to do anything.  If for some reason, you don't want to use this
capability (perhaps the POSIX 2008 support is buggy on your system), you
can manually compile Perl to use the old non-thread-safe implementation
by passing the argument C<-Accflags='-DNO_THREAD_SAFE_LOCALE'> to
XXX
F<Configure>.  Except on Windows, this will continue to use certain of
the POSIX 2008 functions in some situations.  If these are buggy, you
can pass the following to F<Configure> instead or additionally:
...
The locale will change in all other threads at the
thread
Prior to Perl 5.34 or on a system that does not support
