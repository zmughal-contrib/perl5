...
Perl source plays by ANSI C89 rules: with some C99  extensions.
...
XXX allowed by many C compilers but cranking up the ANSI C89 strictness
...
That is XXX C99 or C++.  Some C compilers allow that, but you shouldn't.
...
That is XXX C99 or C++.  While it would indeed be awfully nice to have that
