...
Perl_utf8_to_bytes(pTHX_ U8 *s, STRLEN *lenp)
{
...

    {
        ...
        if (*lenp > 5 * PERL_WORDSIZE) {
            ...
            /* First, go through the string a word at-a-time to verify that it
             * is downgradable.  If it contains start bytes besides C2 and C3,
             * then it isn't. */
            ...
            /* Points to the first byte >=s which is positioned at a word
             * boundary.  If s is on a word boundary, it is s, otherwise it is
             * to the next word. */
            U8 * partial_word_end = s + PERL_WORDSIZE * PERL_IS_SUBWORD_ADDR(s)
                                    - (PTR2nat(s) & PERL_WORD_BOUNDARY_MASK);

            /* Here there is at least a full word beyond the first word
             * boundary.  Process up to that boundary. */
            ...
            do {
                 ...
                 * becomes something else.  Then XORing with C0 causes any
                 * start byte to become 0; all other bytes non-zero. */
                PERL_UINTMAX_T start_bytes
                              = ((* (PERL_UINTMAX_T *) s) & C0_mask) ^ C0_mask;

                /* These shifts causes the most significant bit to be set to 1
                 * for any bytes in the word that aren't completely 0.  Hence
                 * after these, only the start bytes have 0 in their msb */
            ...
            /* If the final byte was a start byte, we are out of sync for the
             * loop below, so back off one (we would have failed and not gotten
             * here if that start byte hadn't yielded a 2-byte sequence) */
         ...
         * well-formed sequences are downgradable.  But we didn't test that
         * there weren't two C2's in a row, for example.  That means that in
